...
appendAll s xs = modifyIORef s (++concat xs)
...
swapIORefs x y = do temp <- readIORef x
                    y'   <- readIORef y
                    writeIORef x y'
                    writeIORef y temp
...
hSelectLines h nums = do
    lines <- hFetchLines h
    return [j | (i,j) <- zip [1..] lines, i `elem` nums]

-- hSelectLines' :: Handle -> [Int] -> Int -> [String] -> IO [String]
-- hSelectLines' h nums idx selectedLines = do
--     eof <- hIsEOF h
--     if eof then return selectedLines
--     else do
--         l <- hGetLine h
--         if   idx `elem` nums
--         then hSelectLines' h nums (idx+1) (selectedLines++[l])
--         else hSelectLines' h nums (idx+1) selectedLines



-- hSelectLines :: Handle -> [Int] -> IO [String]
-- hSelectLines h nums = hSelectLines' h nums 1 []
...
interact' f state = do
    line <- getLine
    let (bool, string, newstate) = f (line, state)
    putStrLn string
    if bool then
        interact' f newstate
