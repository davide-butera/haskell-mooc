...
oddFactorial n = product [x | x <- [1..n], odd x]
...
myGcd a 0 = a
myGcd 0 b = b
myGcd a b = myGcd (max a b - min a b) (min a b)
...
countdownHelper :: (Eq a, Num a, Show a) => a -> [Char] -> [Char]
countdownHelper 0 result = result
countdownHelper n result = countdownHelper (n-1) (result ++ show n ++ "... ")
------------------------------------------------------------------------------
...
smallestDivisorHelper n k = if mod n k == 0
                            then k
                            else smallestDivisorHelper n (k+1)
------------------------------------------------------------------------------
