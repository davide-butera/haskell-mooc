...
treeMax Empty               = 0
treeMax (Node a left right) = maximum [a, treeMax left, treeMax right]
...
allValues condition (Node a left right) = condition a &&
...
cull val (Node a left right)
  | a == val = Empty
  | otherwise = Node a (cull val left) (cull val right)
...
isOrdered (Node root left right) = allValues (<root) left &&
...
walk (x:xs) (Node root left right) = case x of
  StepL -> walk xs left
  StepR -> walk xs right
...
set (x:xs) val (Node root left right) = case x of
  StepL -> Node root (set xs val left) right
  StepR -> Node root left (set xs val right)
...
search val (Node root left right) = if val == root then Just []
    else case search val left of
                Just x -> Just (StepL:x)
