...
rgb (Mix a b)  = zipWith (\ a b -> (a + b) / 2) (rgb a) (rgb b)
                --map average $ zip (rgb a) (rgb b)
...
toNat n = if n<=0 then Nothing
          else Just $ PlusOne $ toNatHelper $ toNat $ n-1

toNatHelper :: Maybe Nat -> Nat
toNatHelper Nothing  = Zero
toNatHelper (Just n) = n
...
fromBinHelper acc End   = 0
fromBinHelper acc (O b) = fromBinHelper (acc+1) 　b
fromBinHelper acc (I b) = 2^acc + fromBinHelper (acc+1)　b
