...
count x list = length . filter (==x) $ toList list
...
inBoth a b = toList a `intersect` toList b
...
instance Foldable List where
  foldr f initialValue = go
          where
            go Empty       = initialValue
            go (LNode x l) = f x (go l)
...
instance Foldable TwoList where
    foldr f initialValue = go
          where
            go TwoEmpty        = initialValue
            go (TwoNode a b l) = (f a . f b) (go l)
...
instance Functor Fun where
  ...
  --fmap g (Fun f) = Fun (g . runFun (Fun f))
...
sumTree = go mempty
    where
        go sum Leaf         = sum
        go sum (Node x l r) = go sum l <> x <> go sum r
